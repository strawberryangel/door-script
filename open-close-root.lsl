...
// Assumption: The global location of the door doesn't move 
...
//
// This is intended for stand-alone (root) objects. 
// Most doors aren't wandering around but return to their starting point.
...
move_door(integer is_opening)
{
    ...
    while (current_time < open_time) {
        ...
        llSetLinkPrimitiveParamsFast(link_number, [
            PRIM_ROTATION, calculate_rotation(angle),
            PRIM_POSITION, calculate_position(angle)
        ]);
...
move_door_finish(vector target_position, rotation target_rotation)
{
    ...
    llSetLinkPrimitiveParamsFast(link_number, [
        PRIM_ROTATION, target_rotation,
        PRIM_POSITION, target_position,
        PRIM_PHANTOM, FALSE
    ]);
}
